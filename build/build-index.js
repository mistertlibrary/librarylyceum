 ...
 *   db-keywords.csv               -> each database's keywords and tier
 *   topics.csv                    -> the topic map, carried beside the records
...
/* Keywords live in their own file so that databases.csv stays the hand-kept
 * list it has always been, and the gate's baseline does not churn every time a
 * keyword changes. Reviewed in the Search Workbench; only kept terms are here.
 * Tier ("specialist" or "general") orders the no-match suggestions. */
function readDatabaseKeywords() {
  const file = path.join(ROOT, "data", "db-keywords.csv");
  const map = new Map();
  if (!fs.existsSync(file)) { warnings.push("no db-keywords.csv; database records carry no keywords"); return map; }
  for (const r of parseCsv(fs.readFileSync(file, "utf8"))) {
    if (!r.Name) continue;
    map.set(r.Name.toLowerCase(), { keywords: pipeList(r.Keywords), tier: r.Tier || "" });
  }
  return map;
}
...
function readDatabases() {
  ...
  for (const r of rows) {
    ...
    out.push({
      ...
      primary: r.Primary || "",
      ...(kw.has(key) ? { keywords: kw.get(key).keywords, tier: kw.get(key).tier } : {})
    });
  }
  for (const name of kw.keys()) {
    if (!seen.has(name)) warnings.push(`db-keywords.csv: "${name}" matches no row in databases.csv`);
  }
  return out;
}

/* The topic map is consulted only when a search finds nothing: a topic a
 * student might type, and the databases to suggest for it.
 *
 * Which databases, and in what order, is worked out here rather than in the
 * browser. Every database sharing a subject with the topic is a candidate; a
 * candidate's standing is how many topics in the same section of the map it
 * already answers through its own keywords or description. So for a US-history
 * topic the shelf leads with the databases that answer the most US-history
 * topics, not with whichever comes first alphabetically. Ties go to
 * specialists, then to databases whose primary subject is the topic's own,
 * then to spreadsheet order. The result is written into the index, where it can
 * be read and checked. */
function readTopics(databases) {
  const file = path.join(ROOT, "data", "topics.csv");
  if (!fs.existsSync(file)) return [];
  const rows = parseCsv(fs.readFileSync(file, "utf8"))
    .filter(r => r.Topic)
    .map(r => ({ t: r.Topic.toLowerCase(), s: pipeList(r.Subjects), section: r.Section || "" }));

  const answers = (db, topic) =>
    (db.keywords || []).some(k => k.toLowerCase() === topic) ||
    (db.text || "").toLowerCase().includes(topic);

  const standing = new Map();
  for (const db of databases) {
    const bySection = {};
    for (const r of rows) if (answers(db, r.t)) bySection[r.section] = (bySection[r.section] || 0) + 1;
    standing.set(db, bySection);
  }

  return rows.map(r => {
    const lead = databases
      .map((db, i) => ({ db, i }))
      .filter(x => (x.db.subjects || []).some(s => r.s.includes(s)))
      .sort((a, b) =>
        ((standing.get(b.db)[r.section] || 0) - (standing.get(a.db)[r.section] || 0)) ||
        ((a.db.tier === "specialist" ? 0 : 1) - (b.db.tier === "specialist" ? 0 : 1)) ||
        ((r.s.includes(a.db.primary) ? 0 : 1) - (r.s.includes(b.db.primary) ? 0 : 1)) ||
        (a.i - b.i))
      .slice(0, 6)
      .map(x => x.db.title);
    return { t: r.t, s: r.s, d: lead };
  });
}
...
const topics = readTopics(records.filter(r => r.type === "database"));
...
if (mode === "check") {
  ...
  const strip = s => { try { const o = JSON.parse(s); return JSON.stringify([o.records, o.topics || []]); } catch (e) { return null; } };
  if (strip(current) !== JSON.stringify([records, topics])) {
  ...
  console.log(
    ...
    `plus ${sectionStats.sectionCount} sections on ${sectionStats.withSections} guide(s), ${topics.length} topics.`
  );
}
